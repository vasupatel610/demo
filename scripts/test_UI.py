...
if query:  # Only run a search if the query is not empty
    ...
    if not hits:
        ...
        <div class="results-counter">
            🎯 Found {len(hits)} matching products • Showing top {min(top_k, len(hits))}
        </div>
        """, unsafe_allow_html=True)
...
<div style="text-align: center; color: #7f8c8d; padding: 2rem; border-top: 1px solid #ecf0f1; margin-top: 3rem;">
    <p>🛍️ Smart Product Search • Powered by AI • Built with ❤️</p>
</div>
""", unsafe_allow_html=True)
...
# # -------------------------------------------------------------------
# # 3a. Live Suggestions (Autocomplete style)
# # -------------------------------------------------------------------
# suggestions = []
# if query:
#     try:
#         resp = requests.get(SUGGEST_EP, params={"query": query}, timeout=10)
#         if resp.status_code == 200:
#             suggestions = resp.json()
#     except Exception as e:
#         st.error(f"Suggestion fetch failed: {e}")

# # ✅ Replace multiple buttons with a dropdown
# if suggestions:
#     selected = st.selectbox(
#         "Suggestions", 
#         options=["(Keep my typed query)"] + suggestions,
#         key="suggestion_select"
#     )
#     if selected != "(Keep my typed query)":
...
#         h["local_score"] = overlap + 2 * name_overlap
...
#         for i, h in enumerate(hits, 1):
#             st.markdown(f"---\n### {i}. " + highlight_text(h.get('product_name', ""), query), unsafe_allow_html=True)
#             col1, col2 = st.columns([1, 4])
...
#             with col2:
#                 st.markdown(highlight_text(h.get('product_description', "—"), query), unsafe_allow_html=True)
